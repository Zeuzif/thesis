...
\begin{enumerate}
	...
	\end{itemize}
\end{enumerate}
\fig{architecture.png}{architecture de l'application}{0.7}
\clearpage
...
\label{ssub:outils}
\begin{itemize}
	\itemb \textbf{Xcode} : un environnement de développement (IDE)\nomenclature{IDE}{Integrated development environment.} pour macOS, Fournit toute une suite logicielle (graphiques, audio, etc.) pour développeurs et programmeurs. Il permet de créer des logiciels pour iOS, watchOS et tvOS.
...
\subsubsection{Application native} % (fold)
...
\subsubsection{API Rest\nomenclature{API}{Application Programming Interface.}} % (fold)
\label{ssub:api_rest}
Comme il a été dit dans la partie spécification des besoins notre application devrait être portable et extensible. Les services web REST permet au mieux de répondre à ses exigences et sont plus faciles à implémenter, comparés aux méthodes du type RPC\nomenclature{RPC}{Remote Procedure Call.} (Remote Procedure Call) comme SOAP.
...
\begin{itemize}
	\item Le projet MySQL a rendu son code source disponible sous les termes de la GNU\nomenclature{GNU}{General Public License.} (General Public License), donc il représente une solution à moindre coût, l'autre avantage est qu'il y a une grande communauté derrière, par conséquent, nous pouvons facilement apprendre et dépanner MySQL à partir de différentes sources comme des blogs, des livres blancs et des livres.
	\item Nous n'avons pas de données sur le nombre d'utilisateur à venir, donc la mise à l'échelle et la réplication sont des fonctions primordiales et notre SGBD\nomenclature{SGBD}{Système de Gestion de Base de Données.} les prennent  en charge.
...
\begin{figure}%
    \centering
    \subfloat[L’accueil]{{\includegraphics[width=7cm]{figs/ui1.png} }}%
    \qquad
    \subfloat[Ajouter offre planifiée]{{\includegraphics[width=7cm]{figs/ui2.png} }}%
...
\begin{figure}%
    \centering
    \subfloat[Conducteur]{{\includegraphics[width=7cm]{figs/ui3.png} }}%
    \qquad
    \subfloat[Passager]{{\includegraphics[width=7cm]{figs/ui4.png} }}%
...
\begin{figure}%
    \centering
    \subfloat[Authentification]{{\includegraphics[width=7cm]{figs/ui6.png} }}%
    \qquad
    \subfloat[Profil]{{\includegraphics[width=7cm]{figs/ui5.png} }}%
    \caption{Utilisateur}%
    % \label{fig:example}%
\end{figure}
\clearpage
\begin{figure}%
    \centering
    \subfloat[Informations personnelles]{{\includegraphics[width=7cm]{figs/ui7.png} }}%
    \qquad
    \subfloat[Vérification du téléphone]{{\includegraphics[width=7cm]{figs/ui8.png} }}%
